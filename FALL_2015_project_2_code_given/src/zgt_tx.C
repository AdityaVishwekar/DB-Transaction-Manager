...
void *readtx(void *arg){
  ...
  zgt_tx *transactionptr=get_tx(node->tid);
  if(transactionptr != NULL){
    transactionptr->print_tm();
    if(transactionptr->status == TR_ABORT){//tx is aborted
      fprintf(logfile, "T%d\t%c \tReadTx\n", node->tid, node->Txtype);  // Write log record and close
      fflush(logfile);
      zgt_v(0);       // Release tx manager 
      transactionptr->print_tm();  
    }
    else if(transactionptr->status == TR_ACTIVE){//Tx is active
      zgt_v(0);       // Release tx manager 
      int lockfound=transactionptr->set_lock(node->tid, transactionptr->sgno,node->obno, node->count,'S');//check if lock available
      //above statement will return '0' if there is lock availa 
      if(lockfound == 0){//Lock is obtain for tx
        transactionptr->perform_readWrite(transactionptr->tid,node->obno,'S');
      }
    }
  else{
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  if(transactionptr != NULL){
    ...
    if(transactionptr->status == TR_ABORT){//tx is aborted
      ...
      zgt_v(0);       // Release tx manager
      int lockfound=transactionptr->set_lock(node->tid, transactionptr->sgno,node->obno, node->count,'X');//check is lock available
      if(lockfound == 0){//Lock is obtain for tx
        transactionptr->perform_readWrite(transactionptr->tid,node->obno,'X');
      }
    }
...
void *committx(void *arg)
{
 //remove the locks before committing
    struct param *node = (struct param*)arg;// get tid and count
//Release all locks on every object hold by that transaction before committing
  start_operation(node->tid, node->count);
...
void *do_commit_abort(long t, char status){
  ...
  if(txptr != NULL){
     txptr->free_locks(); //Release all locks on every object hold by that transaction
     zgt_v(t);//Release the transaction
...
{  //if the thread has to wait, block the thread on a semaphore from the
  ...
  //write your code
    ...
    zgt_p(0);       // Lock Tx manager
      ...
      zgt_hlink* objNode = ZGT_Ht->find(sgno1, obno1); // search for the object in the lock table

      if (objNode == NULL)  // if object not present
      {
        ZGT_Ht->add(txNode,sgno1, obno1,lockmode1);//add to the hash table
        lockGranted = true;
        zgt_v(0);         // Release tx manager 
      }
  else  // else object present
  {
    ...
    if(lockmode1=='S') 
     {
      if(objNode -> tid != tid1)  //Currently locked by another tx
      {
          // zgt_tx* txNode1 = get_tx(objNode -> tid); // search for the transaction object

        if (get_tx(objNode->tid) -> Txtype == 'R' && txNode -> Txtype == 'R')  // both are read only
        {
...
//              objNode -> nextp = txNode -> head;
          ...
          {
          txNode->setTx_semno(objNode->tid, objNode->tid);//Set semaphore on tx
          printf("Tx %d is waiting on:T%d\n", txNode->tid,objNode->tid);
          // wait_for* w=new wait_for();
          // w->wait_for::deadlock();
          zgt_v(0);       // Release tx manager
